...
// Komponen untuk mengelola Narasi/Bacaan (soal cerita)
// Guru pilih: soal mandiri (tanpa narasi), narasi baru, atau ikut narasi lama
function NarasiSection({ form, setForm, soalList }) {
  const [expanded, setExpanded] = useState(!!form.narasi_id);

  // Kumpulkan narasi yang sudah ada di soalList (unik per narasi_id)
  const narasiTersedia = [];
  const seen = new Set();
  soalList.forEach(s => {
    if (s.narasi_id && s.narasi && !seen.has(s.narasi_id)) {
      seen.add(s.narasi_id);
      narasiTersedia.push({ narasi_id: s.narasi_id, narasi: s.narasi });
    }
  });

  // Soal-soal yang pakai narasi tertentu
  const hitungSoalPakaiNarasi = (nid) => soalList.filter(s => s.narasi_id === nid).length;

  const setMode = (mode) => {
    if (mode === "mandiri") {
      setForm(p => ({ ...p, narasi: "", narasi_id: "" }));
      setExpanded(false);
    } else if (mode === "baru") {
      const newId = "narasi_" + Date.now();
      setForm(p => ({ ...p, narasi: "", narasi_id: newId }));
      setExpanded(true);
    }
  };

  const pakaiNarasiLama = (n) => {
    setForm(p => ({ ...p, narasi: n.narasi, narasi_id: n.narasi_id }));
    setExpanded(true);
  };

  // Cek apakah narasi_id sekarang adalah narasi yang sudah dipakai soal lain
  const isPakaiNarasiLama = form.narasi_id && narasiTersedia.some(n => n.narasi_id === form.narasi_id) && hitungSoalPakaiNarasi(form.narasi_id) > 0;

  if (!expanded && !form.narasi_id) {
    // Tombol singkat untuk membuka section
    return (
      <div style={{marginBottom:"16px"}}>
        <button
          type="button"
          onClick={() => setExpanded(true)}
          style={{background:"#f0f9ff", border:"1px dashed #93c5fd", borderRadius:"var(--radius2)", padding:"10px 14px", fontSize:"13px", color:"#1e40af", cursor:"pointer", fontWeight:"600", width:"100%", textAlign:"left"}}
        >
          📖 Tambah Narasi/Bacaan (untuk soal cerita) — Opsional
        </button>
      </div>
    );
  }
  ...
  return (
    <div style={{background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:"var(--radius2)", padding:"14px", marginBottom:"16px"}}>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"10px"}}>
        <div style={{fontSize:"13px", fontWeight:"700", color:"#0369a1"}}>📖 Narasi/Bacaan untuk Soal Cerita</div>
        <button
          type="button"
          onClick={() => { setMode("mandiri"); }}
          style={{background:"transparent", border:"none", color:"#0369a1", fontSize:"12px", cursor:"pointer", fontWeight:"600"}}
        >✕ Soal Mandiri (tanpa narasi)</button>
      </div>

      {/* Pilih: narasi baru atau ikut narasi lama */}
      {narasiTersedia.length > 0 && !isPakaiNarasiLama && !form.narasi_id && (
        <div style={{marginBottom:"10px"}}>
          <div style={{fontSize:"12px", color:"#075985", marginBottom:"6px", fontWeight:"600"}}>Ada {narasiTersedia.length} narasi di ujian ini. Pakai yang sudah ada?</div>
          {narasiTersedia.map((n, i) => (
            <button
              key={i}
              type="button"
              onClick={() => pakaiNarasiLama(n)}
              style={{display:"block", width:"100%", textAlign:"left", padding:"8px 10px", marginBottom:"4px", background:"white", border:"1px solid #bae6fd", borderRadius:"6px", cursor:"pointer", fontSize:"12px", color:"#0c4a6e"}}
            >
              <strong>📚 Narasi {i+1}</strong> <span style={{color:"#64748b"}}>(dipakai oleh {hitungSoalPakaiNarasi(n.narasi_id)} soal)</span><br/>
              <span style={{color:"#475569"}}>{n.narasi.substring(0, 100)}{n.narasi.length > 100 ? "..." : ""}</span>
            </button>
          ))}
          <button
            type="button"
            onClick={() => setMode("baru")}
            style={{display:"block", width:"100%", padding:"8px 10px", marginTop:"6px", background:"#dbeafe", border:"1px dashed #2563eb", borderRadius:"6px", cursor:"pointer", fontSize:"12px", color:"#1e40af", fontWeight:"600"}}
          >+ Atau buat narasi baru</button>
        </div>
      )}

      {/* Indikator pakai narasi lama */}
      {isPakaiNarasiLama && (
        <div style={{marginBottom:"8px", padding:"8px 10px", background:"#dcfce7", border:"1px solid #86efac", borderRadius:"6px", fontSize:"12px", color:"#166534"}}>
          ✓ Soal ini akan ikut <strong>Narasi yang sudah ada</strong> (dipakai {hitungSoalPakaiNarasi(form.narasi_id)} soal lain). Tidak perlu ketik narasi lagi.
        </div>
      )}

      {/* Textarea narasi (hanya untuk narasi baru) */}
      {form.narasi_id && !isPakaiNarasiLama && (
        <div>
          <label style={{fontSize:"12px", fontWeight:"600", color:"#075985", display:"block", marginBottom:"4px"}}>
            Teks Bacaan/Cerita
          </label>
          <textarea
            value={form.narasi}
            onChange={e => setForm(p => ({ ...p, narasi: e.target.value }))}
            placeholder="Tulis bacaan/cerita di sini. Contoh: 'Pada zaman dahulu, ada seorang anak bernama Malin Kundang...'"
            rows={6}
            style={{width:"100%", padding:"10px", border:"1px solid #bae6fd", borderRadius:"6px", fontSize:"13px", fontFamily:"inherit", resize:"vertical"}}
          />
          <div style={{fontSize:"11px", color:"#075985", marginTop:"4px", fontStyle:"italic"}}>
            💡 Setelah simpan soal pertama, narasi ini akan muncul sebagai pilihan saat Anda buat soal berikutnya — jadi tidak perlu ketik ulang.
          </div>
        </div>
      )}
    </div>
...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar { width: 260px; background: var(--navy); min-height: 100vh; padding: 24px 0; display: flex; flex-direction: column; position: fixed; top: 0; left: 0; z-index: 100; transition: transform 0.25s ease; }
  ...
  .hamburger-btn { display: none; position: fixed; top: 12px; left: 12px; z-index: 200; background: var(--navy); color: white; border: none; width: 42px; height: 42px; border-radius: 10px; font-size: 22px; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.2); align-items: center; justify-content: center; }
  ...
  @media (max-width: 768px) {
    ...
    /* Tabel jadi scrollable horizontal */
    .table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; }
    .table-wrap table { min-width: 600px; }
    /* Form layout jadi 1 kolom */
    .form-grid { grid-template-columns: 1fr !important; }
    /* Tombol di tabel/header lebih compact */
...
export default function App() {
  ...
  // Pastikan viewport meta tag ada (untuk responsive mobile)
  useEffect(() => {
...
function GuruDashboard({ guru, onLogout }) {
  ...
  // Saat ganti halaman di mobile, tutup sidebar otomatis
  const gantiPage = (id) => {
    setPage(id);
  ...
  return (
    <div className="dashboard">
      ...
      {/* Overlay untuk tutup sidebar saat tap di luar */}
      <div
      ...
      <div className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="sidebar-logo">
          <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"8px"}}>
            <img src="data:image/jpeg;base64," alt="Logo" style={{width:"40px",height:"40px",borderRadius:"50%",objectFit:"cover"}} />
            <div>
              <h2 style={{fontSize:"14px",lineHeight:"1.2"}}>CBT<span style={{color:"var(--blue)"}}> SMPN 7</span></h2>
              <p style={{fontSize:"10px",marginTop:"2px"}}>Tanah Sepenggal</p>
            </div>
          </div>
        ...
        <div className="sidebar-menu">
          {menuItems.map(m => (
            <button key={m.id} className={page === m.id ? "active" : ""} onClick={() => gantiPage(m.id)}>
              {m.icon} {m.label}
            </button>
          ))}
        </div>
...
function SoalPage({ ujianList, onRefresh }) {
  ...
  const [form, setForm] = useState({ pertanyaan: "", gambar: "", narasi: "", narasi_id: "", opsi: ["","","",""], jawaban: 0 });
  ...
  const [notifSave, setNotifSave] = useState(null); // notif setelah simpan soal
  const fileInputRef = useRef(null);
  const formAreaRef = useRef(null);

  // Auto-hilangkan notif setelah 3 detik
  useEffect(() => {
    if (!notifSave) return;
    const t = setTimeout(() => setNotifSave(null), 3000);
  ...
  const resetForm = () => {
    setForm({ pertanyaan: "", gambar: "", narasi: "", narasi_id: "", opsi: ["","","",""], jawaban: 0 });
    setEditSoal(null);
    setErrorGambar("");
    setProgressGambar(0);
  };
...
<style>
    ...
    try {
      if (editSoal) {
        ...
        if (useDemo) {
          ...
          await supabase(`soal?id=eq.${editSoal.id}`, { method: "PATCH", body: JSON.stringify({ pertanyaan: form.pertanyaan, gambar: form.gambar || null, narasi: form.narasi || null, narasi_id: form.narasi_id || null, opsi: form.opsi, jawaban: form.jawaban }) });
        }
        await onRefresh();
        setSoalList(prev => prev.map(s => s.id === editSoal.id ? { ...s, ...form, opsi: [...form.opsi] } : s));
        resetForm();
        setNotifSave({ type: "edit", waktu: Date.now() });
      } else {
        // MODE TAMBAH BARU
        ...
        if (useDemo) {
          ...
          await supabase("soal", { method: "POST", body: JSON.stringify({ ujian_id: Number(selectedUjian), pertanyaan: form.pertanyaan, gambar: form.gambar || null, narasi: form.narasi || null, narasi_id: form.narasi_id || null, opsi: form.opsi, jawaban: form.jawaban }) });
        }
        await onRefresh();
        setSoalList(prev => [...prev, soalBaru]);
        // Reset form tapi PERTAHANKAN selectedUjian — guru bisa langsung lanjut soal berikutnya
        resetForm();
        setNotifSave({ type: "tambah", waktu: Date.now() });
        // Scroll ke form supaya guru langsung lihat field pertanyaan kosong (siap soal berikutnya)
        setTimeout(() => {
          if (formAreaRef.current) {
            formAreaRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
            // Auto-focus ke textarea pertanyaan
            const ta = formAreaRef.current.querySelector("textarea");
            if (ta) ta.focus();
          }
        }, 100);
      }
  ...
  // 1. Pertanyaan / 1) Pertanyaan / - Pertanyaan (bullet list dari Word)
  // A. Opsi / a. Opsi / A) Opsi / - Opsi (bullet)
  // Jawaban: A / **Jawaban: A** / Kunci: A / Jwb: A / Answer: A
  const parseTextToSoal = (text) => {
    // STEP 1: Bersihkan markdown bold/italic dari mammoth (**, *, __, _)
    // Tapi simpan kontennya
    const cleanMarkdown = (s) => s
      .replace(/\*\*\*\*/g, "")           // **** (artifact dari Word formatting)
      .replace(/\*\*([^*]+)\*\*/g, "$1")  // **bold** → bold
      .replace(/__([^_]+)__/g, "$1")      // __bold__ → bold
      .replace(/\*([^*]+)\*/g, "$1")      // *italic* → italic
      .replace(/_([^_]+)_/g, "$1");       // _italic_ → italic

    // STEP 2: Bersihkan & split baris
    const lines = text.split(/\r?\n/)
      .map(l => cleanMarkdown(l).trim())
      .filter(Boolean);

    // STEP 3: Deteksi tipe tiap baris
    // Jenis baris:
    //  - "kunci"   → baris berisi jawaban (mis. "Jawaban: A")
    ...
    // STEP 4: Parsing state machine
    const hasil = [];
    let current = null; // { pertanyaan, opsi: [], jawaban }
    let bulletBuffer = []; // tampung baris bullet sebelum kunci jawaban muncul

    const commitCurrent = () => {
      if (!current) return;
      // Kalau bulletBuffer punya isi & opsi kosong, anggap 5 bullet pertama adalah:
      // bullet[0] = pertanyaan (kalau current.pertanyaan kosong)
      // bullet[1..4] = opsi A-D
      if (bulletBuffer.length > 0 && current.opsi.length === 0) {
        if (!current.pertanyaan && bulletBuffer.length >= 5) {
          current.pertanyaan = bulletBuffer[0];
          current.opsi = bulletBuffer.slice(1, 5);
        } else if (current.pertanyaan && bulletBuffer.length >= 4) {
          current.opsi = bulletBuffer.slice(0, 4);
        }
      }
      if (current.pertanyaan && current.opsi.length >= 2) {
        while (current.opsi.length < 4) current.opsi.push("-");
        hasil.push({
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const matchPertanyaanNomor = line.match(POLA_PERTANYAAN_NOMOR);
      if (matchPertanyaanNomor) {
        commitCurrent();
        current = { pertanyaan: matchPertanyaanNomor[2].trim(), opsi: [], jawaban: 0 };
        bulletBuffer = [];
        continue;
      }

      // Cek bullet (mode Word list)
      const matchBullet = line.match(POLA_BULLET);
      if (matchBullet) {
        const isi = matchBullet[1].trim();
        // Skip bullet yang sepertinya placeholder gambar (mis. "Gambar air laut")
        // Pattern: bullet pendek yang dimulai dengan "Gambar"/"Image"/"Foto" dan tidak diakhiri tanda baca
        const placeholder = /^(gambar|image|foto|picture)\b/i.test(isi) && isi.length < 60 && !isi.endsWith(".") && !isi.endsWith("?");
        if (placeholder) continue; // abaikan placeholder gambar
        // Kalau belum ada pertanyaan dan ini bullet pertama setelah commit,
        // mulai soal baru dengan bullet ini sebagai pertanyaan.
        if (!current) {
          current = { pertanyaan: "", opsi: [], jawaban: 0 };
          bulletBuffer = [isi];
        } else {
          bulletBuffer.push(isi);
        }
        continue;
      }

      // Selain itu: anggap pertanyaan biasa (kalau belum ada current)
      // atau lanjutan baris sebelumnya
      if (!current) {
        ...
        if (current.opsi.length === 0 && bulletBuffer.length === 0) {
          current.pertanyaan += " " + line;
        }
      }
    }
    ...
    <>
      ...
      {selectedUjian && (
        <>
          ...
          {/* MODE: MANUAL */}
          {uploadMode === "manual" && (
            <div ref={formAreaRef} className="card" style={{border: editSoal ? "2px solid var(--yellow)" : "1px solid var(--border)"}}>
              <div className="card-header">
                <h2>{editSoal ? "✏️ Edit Soal" : `✏️ Tambah Soal Manual ${soalList.length > 0 ? `(Soal ke-${soalList.length + 1})` : ""}`}</h2>
              ...
              {/* Notif sukses simpan */}
              {notifSave && !editSoal && (
                <div style={{background:"var(--green3)", color:"var(--green2)", borderRadius:"var(--radius2)", padding:"10px 14px", marginBottom:"16px", fontSize:"13px", fontWeight:"600", display:"flex", alignItems:"center", gap:"8px"}}>
                  ✅ {notifSave.type === "edit" ? "Soal berhasil diperbarui" : `Soal ke-${soalList.length} berhasil disimpan! Silakan ketik soal berikutnya di bawah ini.`}
                </div>
              )}

              {/* SECTION: NARASI/BACAAN (opsional, untuk soal cerita) */}
              <NarasiSection
                form={form}
                setForm={setForm}
                soalList={soalList}
          ...
          {uploadMode === "word" && (
            <div className="card">
              ...
              {/* Panduan format */}
              <div style={{background:"var(--blue3)", borderRadius:"var(--radius2)", padding:"16px", marginBottom:"20px"}}>
                <div style={{fontSize:"13px", fontWeight:"700", color:"var(--blue2)", marginBottom:"8px"}}>📋 Format Word yang Didukung</div>
                <div style={{fontSize:"12px", color:"var(--navy3)", lineHeight:"1.8", marginBottom:"10px"}}>
                  Sistem cerdas — mendukung <b>2 cara penulisan</b>:
                </div>

                {/* Format 1: Manual */}
                <div style={{fontSize:"11px", fontWeight:"700", color:"var(--blue2)", marginTop:"10px", marginBottom:"4px"}}>✅ Cara 1: Penomoran Manual</div>
                <div style={{fontSize:"12px", background:"white", borderRadius:"6px", padding:"10px 12px", fontFamily:"var(--mono)", lineHeight:"1.7"}}>
                ...
                {/* Format 2: Bullet/Numbered list Word */}
                <div style={{fontSize:"11px", fontWeight:"700", color:"var(--blue2)", marginTop:"12px", marginBottom:"4px"}}>✅ Cara 2: Bullet/Numbered List Word</div>
                <div style={{fontSize:"12px", background:"white", borderRadius:"6px", padding:"10px 12px", fontFamily:"var(--mono)", lineHeight:"1.7"}}>
                  <div>• Ibu kota Indonesia adalah?</div>
                  <div>• Bandung</div>
                  <div>• Jakarta</div>
                  <div>• Surabaya</div>
                  <div>• Medan</div>
                ...
                <div style={{fontSize:"11px", color:"var(--gray)", marginTop:"6px"}}>
                  💡 Bullet pertama otomatis jadi pertanyaan, bullet 2-5 jadi opsi A-D.
                </div>

                <div style={{fontSize:"11px", fontWeight:"700", color:"var(--blue2)", marginTop:"12px", marginBottom:"4px"}}>🔑 Format Kunci Jawaban (semua didukung):</div>
                <div style={{fontSize:"11px", color:"var(--navy3)", lineHeight:"1.8"}}>
                  <code>Jawaban: A</code> · <code>Kunci: B</code> · <code>Jwb: C</code> · <code>Answer: D</code> · <b>format tebal/bold juga OK</b>
                </div>
              </div>
    ...
    <div className="cbt-wrap" style={{userSelect:"none"}}>
      ...
      <div className="cbt-body">
        <div className="cbt-main">
          {(() => {
            // Helper: ambil teks narasi untuk soal saat ini
            // Kalau soal current pakai narasi_id, cari teks narasi dari soal manapun yang punya narasi_id sama
            const cur = soal[current];
            if (!cur.narasi_id) return null;
            // Cari narasi: dari soal current sendiri atau dari soal lain yang share narasi_id
            const sumberNarasi = cur.narasi ? cur : soal.find(s => s.narasi_id === cur.narasi_id && s.narasi);
            if (!sumberNarasi || !sumberNarasi.narasi) return null;
            // Hitung urutan soal dalam grup narasi ini (untuk badge "Soal 1 dari 3 untuk narasi ini")
            const soalsDiGrup = soal.filter(s => s.narasi_id === cur.narasi_id);
            const idxDiGrup = soalsDiGrup.findIndex(s => s === cur) + 1;
            return (
              <div className="cbt-narasi-card" style={{background:"#fef3c7", border:"2px solid #fcd34d", borderRadius:"var(--radius)", padding:"20px", marginBottom:"16px"}}>
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"10px"}}>
                  <div style={{fontSize:"13px", fontWeight:"700", color:"#92400e"}}>📖 BACAAN</div>
                  <div style={{fontSize:"11px", color:"#92400e", fontWeight:"600"}}>
                    Soal {idxDiGrup} dari {soalsDiGrup.length} untuk bacaan ini
                  </div>
                </div>
                <div style={{fontSize:"14px", lineHeight:"1.7", color:"#1e293b", whiteSpace:"pre-wrap"}}>
                  <MathText text={sumberNarasi.narasi} />
                </div>
              </div>
            );
          })()}
